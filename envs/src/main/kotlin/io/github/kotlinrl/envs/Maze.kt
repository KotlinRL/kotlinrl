...
class Maze(
    ...
    /**
     * Retrieves a list of all possible states in the maze environment.
     *
     * This method generates a list of all states represented as 1D NDArrays,
     * where each state corresponds to a key in the `maze`. Each key is transformed
     * into a 1D NDArray of integers.
     *
     * @return A list of 1D NDArrays representing all possible states in the maze.
     */
    override fun allStates(): List<NDArray<Int, D1>> = maze.keys.map { mk.ndarray(it.toIntArray()) }

    /**
     * Simulates a step in the environment without altering the internal state.
     *
     * This method calculates the reward for the current state, determines
     * the next state based on the given action, and checks if the episode
     * has terminated by reaching the goal. The result is encapsulated in
     * a `StepResult` object, which includes the updated state, reward,
     * termination status, truncation status, and relevant metadata.
     *
     * @param state The current state represented as a 1D NDArray containing the row and column indices.
     * @param action The action to be performed, represented as an integer (0: UP, 1: RIGHT, 2: DOWN, 3: LEFT).
     * @return A `StepResult` containing the next state, the computed reward,
     *         a termination flag, a truncation flag (always false),
     *         and an empty metadata map.
     */
